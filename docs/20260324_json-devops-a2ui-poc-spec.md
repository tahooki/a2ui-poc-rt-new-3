...
2. 각 플로우는 `기존 콘솔형 UI`와 `AI 템플릿 UI`를 같은 화면에서 비교 가능하게 만든다.
3. 기존 UI는 실제로 복잡하게 구현하지 않고, `복잡하게 느껴지게` 연출한다.
4. 템플릿 UI 쪽은 실제로 동작해야 하며, 버튼 액션에 따라 메모리 상태가 바뀌어야 한다.
...
- 같은 작업이 AI 템플릿 UI 쪽에서 훨씬 단순하게 보인다.
...
- 필드가 많다
- 어려운 용어가 있다
- 이전 배포 정보를 사람이 직접 찾아 입력해야 한다
- 다음 액션이 자동으로 이어지지 않는다

단, 실제 구현은 과하게 하지 않는다.
폼은 많아 보여도 내부 동작은 단순한 상태 변경이면 충분하다.
...
배포를 시작하기 전 준비 과정이 얼마나 번거로운지 보여주고,
동시에 AI 템플릿 UI가 그 준비를 얼마나 압축하는지 보여준다.
...
이 화면은 실제로 모든 값이 유효할 필요는 없다.
입력량이 많고 사람이 직접 채워야 한다는 인상만 주면 된다.
...
- 기존 UI: `사용자가 빌드부터 배포 설정까지 직접 구성해야 한다`
- AI 템플릿 UI: `AI가 이전 성공 배포를 참고해 초안을 구성하고, 사용자는 확인만 한다`
...
승인 자체보다 `승인 판단에 필요한 조사 작업`이 얼마나 번거로운지 보여준다.
...
- 기존 UI: `승인 버튼보다 승인 판단이 더 어렵다`
- AI 템플릿 UI: `AI가 판단 근거를 먼저 모아준다`
...
장애 상황에서 롤백 판단과 실행 흐름을 빠르게 지원한다는 메시지를 전달한다.
...
- 판단 근거와 실행 단계가 분리된다
- 어떤 버전으로 되돌려야 할지 사용자가 직접 판단해야 한다
- 급한 상황인데도 여러 블록을 오가야 한다
...
- 기존 UI: `급할수록 더 많은 화면을 오가야 한다`
- AI 템플릿 UI: `현재 장애 문맥 안에서 바로 판단하고 실행할 수 있다`
...
`페이지 문맥 안에서 의도를 빠르게 라우팅하는 작업 인터페이스`면 충분하다.
...
`기존 DevOps 작업이 왜 불편한지`와 `AI 템플릿 UI가 그 불편을 어떻게 줄이는지`를
짧은 시간 안에 설득력 있게 보여주는 데 집중한다.

따라서 아래 기준을 유지한다.

- 기존 UI는 복잡하게 느껴지게 하되 구현은 단순하게
