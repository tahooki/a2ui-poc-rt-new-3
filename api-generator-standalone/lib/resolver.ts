...
export class Resolver {
  private readonly preservedTypeNames: Set<string>;
  private readonly typeParameterMap: Record<string, string>;
  private readonly maxDepth: number;

  constructor(options: ResolverOptions = {}) {
    this.preservedTypeNames = new Set(
      options.preservedTypes ?? DEFAULT_PRESERVED_TYPES,
    );
    this.typeParameterMap = options.typeParameterMap ?? {};
  ...
  private getPreservedTypeName(type: Type): string | null {
    ...
    if (aliasSymbol) {
      ...
      if (name in this.typeParameterMap) {
        return this.typeParameterMap[name];
      }
    }
